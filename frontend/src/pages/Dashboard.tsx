    ...
    <div>
      ...
      {(ov?.byTrain?.length ?? 0) > 0 && (
        <div className="panel" style={{ marginTop: 4, marginBottom: 22 }}>
          <h3>Disponibilidad por Tren</h3>
          <div className="train-grid">
            {ov.byTrain.map((t: any) => (
              <div key={t.tren} className={'train-card ' + availClass(t.disponibilidad)}>
                <div className="train-name">{t.tren}</div>
                <div className="train-pct">{t.disponibilidad}%</div>
                <div className="train-detail">
                  {t.total} activos · {t.camaras} cámaras
                  {t.caidos > 0 && <span className="train-down"> · {t.caidos} con problema</span>}
                </div>
              </div>
            ))}
          </div>
