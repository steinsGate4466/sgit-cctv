...
/* ---------------------------------------------------------------------------
   LOS COMENTARIOS NO SON CÓDIGO. (bloque 34)
   ---------------------------------------------------------------------------
   Este verificador busca patrones de texto, y hasta ahora los buscaba también
   dentro de los comentarios. Mientras nadie escribió sobre roles en un
   comentario, funcionó por suerte.

   Se rompió el día que se documentó el propio arreglo del bloque 34: la frase
   «antes esto comparaba role === 'Jefe de Mantenimiento'» hizo que el
   verificador denunciara como deuda VIVA la deuda que se acababa de quitar.
   Y una explicación con comillas dentro del array PERMISSIONS convirtió esa
   frase en un permiso inventado, que salió en dos secciones más.

   Un verificador que castiga documentar el porqué acaba enseñando a no
   documentarlo. Así que primero se quitan los comentarios y luego se busca.

   No es un analizador sintáctico: se apoya en que en este proyecto no hay
   expresiones regulares con `//` ni cadenas que abran un bloque de
   comentario. Si algún día las hubiera, esto daría un falso NEGATIVO —por eso
   el cambio se comprueba metiendo un literal de verdad y viendo que salta.
   --------------------------------------------------------------------------- */
function sinComentarios(txt) {
  return txt
    .replace(/\/\*[\s\S]*?\*\//g, ' ')   // de bloque, incluidos los /** */
    .replace(/(^|[^:])\/\/.*$/gm, '$1'); // de línea, sin comerse el // de una URL
}

const leer = (p) => sinComentarios(fs.readFileSync(p, 'utf8'));

// --- 1. El catálogo real: la semilla -----------------------------------------
const seed = leer(path.join(RAIZ_BACK, 'prisma', 'seed.ts'));
...
const catTxt = leer(path.join(RAIZ_BACK, 'src', 'modules', 'roles', 'catalogo-permisos.ts'));
...
/* F) REGLAS ATADAS AL NOMBRE DEL ROL.
   -------------------------------------------------------------------------
   BLOQUE 34: ESTO YA NO ES UN AVISO, ES UN ERROR.

   Estuvo como aviso desde el bloque 12 y sirvió para lo que sirven los
   avisos: se leyó una vez y se dejó ahí. Durante ese tiempo la única regla
   marcada —«Jefe de Mantenimiento» en cinco archivos— era nada menos que la
   segunda llave del borrado definitivo.

   El fallo que esto previene no se ve nunca al probar. Renombrar un rol
   desde la pantalla de Roles no lanza ningún error: los botones desaparecen
   y el servidor rechaza a todo el mundo, o peor, un rol nuevo con el nombre
   correcto hereda una llave que nadie le dio. En ninguno de los dos casos
   hay un mensaje de error que alguien pueda buscar.

   La regla, entonces: el control de acceso se expresa con PERMISOS. Un
   permiso es un registro con identidad propia; el nombre de un rol es una
   etiqueta que el usuario puede reescribir cuando quiera.

   Si algún día hace falta una excepción legítima, se pone aquí con su
   motivo escrito. Una lista corta y explicada es deuda controlada; un aviso
   que nadie mira, no.  */
const EXCEPCIONES_POR_NOMBRE = new Set([
  // (vacía a propósito — bloque 34)
]);

console.log('\nF) Reglas atadas al NOMBRE del rol (prohibido: usa un permiso)');
const porNombreReal = [...porNombre].filter(([n]) => !EXCEPCIONES_POR_NOMBRE.has(n));
if (porNombreReal.length) {
  for (const [n, f] of porNombreReal) {
    fallar(`El rol «${n}» se compara por NOMBRE en ${f.length} archivo(s).`);
    console.log('           ' + f.join(', '));
  }
  console.log('           Renombrar ese rol desde la pantalla de Roles desactiva');
  console.log('           la regla EN SILENCIO. Crea un permiso en el catálogo y');
  console.log('           compruébalo con can()/@RequirePermissions en su lugar.');
} else console.log('   Ninguna. Todo el control de acceso va por permisos.');
